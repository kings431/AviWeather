...
const MetarDisplay: React.FC<MetarDisplayProps> = ({ data, icao, hideRaw = false, hideBadge = false }) => {
  ...
  return (
    <>
      {/* Print-only block: METAR and previous METARs */}
      <div className="print:block hidden mb-8">
        <div className="mb-2">
          <span className="font-bold text-lg">METAR</span>
        </div>
        <pre className="font-mono text-xs whitespace-pre-wrap mb-4">{data.raw}</pre>
        {previousMetars.length > 0 && (
          <div>
            <div className="font-bold mb-1">Previous METARs</div>
            <pre className="font-mono text-xs whitespace-pre-wrap">
              {previousMetars.map((m, idx) => `${m.text}${m.startValidity ? `\n${new Date(m.startValidity).toUTCString()}` : ''}${idx < previousMetars.length - 1 ? '\n\n' : ''}`).join('')}
            </pre>
          </div>
        )}
      </div>
